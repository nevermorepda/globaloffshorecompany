$(document).ready(function() {
	...
	// 	$('.wrap-type').css('display', 'block');
	// 	var p = {};
	// 	p['nation'] = nation;
	// 	$.ajax({
	// 		url: BASE_URL + "/apply-company-services/get-list-service-type-fee",
	// 		type: 'post',
	// 		dataType: 'json',
	// 		data: p,
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data) {
					var t = data[2].length;
					for (var i = 0; i < t; i++) {
						str_type += '<option value="'+data[2][i].id+'">'+data[2][i].name+'</option>';
					}
					$('#type_jurisdictions').html(str_type);
...
$(document).on('change', '#type_jurisdictions', function(event) {
	$('.wrap-loading').css('display', 'block');
	var p = {};
	p['services_tab_fee_id'] = $(this).val();
	$.ajax({
		url: BASE_URL + "/apply-company-services/get-list-service-type-fee",
		type: 'post',
		dataType: 'json',
