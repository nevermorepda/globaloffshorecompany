...
class M_visa_fee extends M_db
{
	...
	{
		$sql   = "SELECT m_country.name, m_country.code FROM {$this->_table} INNER JOIN m_country ON (m_country.id = {$this->_table}.nation_id) WHERE m_country.active = 1 AND {$this->_table}.evisa_1ms <> 0 ORDER BY m_country.name ASC";
		$query = $this->db->query($sql);
		return $query->result();
	}
	...
	function voa_nations()
	{
		...
		$sql  = "SELECT m_country.name, m_country.code FROM {$this->_table} INNER JOIN m_country ON (m_country.id = {$this->_table}.nation_id) WHERE m_country.active = 1";
		$sql .= " AND (".implode(" OR ", $visa_fees).")";
		$sql .= " ORDER BY m_country.name ASC";
