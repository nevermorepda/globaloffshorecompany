...
class M_visa_booking extends M_db
{
	...
	{
		$sql  = "SELECT DISTINCT {$this->_table}.*, U.user_registered FROM {$this->_table} INNER JOIN m_users AS U ON (U.id = {$this->_table}.user_id) WHERE 1=1";
		if (!is_null($info)) {
			if (!empty($info->search_text)) {
				$sql  = "SELECT DISTINCT {$this->_table}.*, U.user_registered FROM {$this->_table} INNER JOIN m_users AS U ON (U.id = {$this->_table}.user_id) INNER JOIN m_visa_pax ON (m_visa_pax.book_id = {$this->_table}.id) WHERE 1=1";
				...
				if ($this->email->valid_email($info->search_text)) {
					$sql .= " AND (UPPER(m_visa_pax.fullname) LIKE '%{$info->search_text}%' OR UPPER(m_visa_pax.passport) LIKE '%{$info->search_text}%' OR {$this->_table}.id = '{$ID}' OR {$this->_table}.order_ref = '{$ID}' OR {$this->_table}.promotion_code LIKE '%{$info->search_text}%' OR UPPER({$this->_table}.primary_email) LIKE '%{$info->search_text}%' OR UPPER({$this->_table}.secondary_email) LIKE '%{$info->search_text}%')";
				} else {
					$sql .= " AND (UPPER(m_visa_pax.fullname) LIKE '%{$info->search_text}%' OR UPPER(m_visa_pax.passport) LIKE '%{$info->search_text}%' OR {$this->_table}.id = '{$ID}' OR {$this->_table}.order_ref = '{$ID}' OR {$this->_table}.promotion_code LIKE '%{$info->search_text}%')";
				}
			}
	...
	function total_bookings($info=NULL, $limit=NULL, $offset=NULL, $status=NULL)
	{
		...
		$sql .= "SELECT DISTINCT {$this->_table}.id FROM {$this->_table} INNER JOIN m_visa_pax ON (m_visa_pax.book_id = {$this->_table}.id) WHERE 1=1";
		if (!is_null($info)) {
			if (!empty($info->search_text)) {
				$info->search_text = strtoupper(trim($info->search_text));
				$ID = preg_replace("/[^0-9]/", "", $info->search_text);
	...
	function total_payments($info=NULL)
	{
		...
		$sql = "SELECT COUNT(*) AS 'total' FROM 
				((SELECT B.id AS 'order_id', B.booking_key AS 'key', 'Apply Visa' AS 'payment_type', B.user_id AS 'customer_id', B.contact_fullname AS 'fullname', B.primary_email, B.secondary_email, B.total_fee AS 'amount', B.payment_method, B.status, B.booking_date AS 'payment_date' FROM {$this->_table} AS B WHERE 1=1 ".$w1." ORDER BY B.booking_date DESC)
				UNION
				(SELECT P.id AS 'order_id', P.payment_key AS 'key', 'Payment Online' AS 'payment_type', '' AS 'customer_id', P.fullname, P.primary_email, P.secondary_email, P.amount, P.payment_method, P.status, P.payment_date AS 'payment_date' FROM m_payment AS P WHERE 1=1 ".$w2." ORDER BY P.payment_date DESC)
	...
	function all_booking_success($info=NULL, $limit=NULL, $offset=NULL)
	{
		...
		$sql = "(SELECT B.id AS 'order_id', B.booking_type_id, '".BOOKING_PREFIX."' AS 'booking_type', '' AS 'fullname', B.primary_email, B.total_fee AS 'amount', B.payment_method, B.status, B.paid_date, B.rush_type, B.arrival_date, B.arrival_port,B.exit_port , B.visa_type, B.fast_checkin, B.car_pickup, B.car_type, B.seats, B.flight_number, B.arrival_time, B.private_visa, B.full_package, B.promotion_code AS 'promotion_code' FROM {$this->_table} AS B WHERE 1=1 ".$w1." ORDER BY B.paid_date ASC)
				UNION
				(SELECT P.id AS 'order_id', '', '".BOOKING_PREFIX_PO."' AS 'booking_type', P.fullname, P.primary_email, P.amount, P.payment_method, P.status, P.paid_date, '' AS 'rush_type', '' AS 'arrival_date', '' AS 'arrival_port','', '' AS 'visa_type', '' AS 'fast_checkin', '' AS 'car_pickup', '' AS 'car_type', '' AS 'seats', '' AS 'flight_number', '' AS 'arrival_time', '' AS 'private_visa', '' AS 'full_package', '' AS 'promotion_code' FROM m_payment AS P WHERE 1=1 ".$w2." ORDER BY P.paid_date ASC)
				UNION
				(SELECT E.id AS 'order_id', '', '".BOOKING_PREFIX_EX."' AS 'booking_type', E.contact_name AS 'fullname', E.primary_email, E.total_fee AS 'amount', E.payment_method, E.status, E.paid_date, '' AS 'rush_type', E.arrival_date, E.arrival_port,'', '' AS 'visa_type', E.fast_checkin, E.car_pickup, E.car_type, E.seats, E.flight_number, '' AS 'arrival_time', '' AS 'private_visa', '' AS 'full_package', '' AS 'promotion_code' FROM m_service_booking AS E WHERE 1=1 ".$w3." ORDER BY E.paid_date ASC)
	...
	{
		$sql   = "SELECT * FROM m_visa_pax WHERE 1 = 1";
	...
	{
		return $this->db->insert("m_visa_pax", $data);
	}
	
	function update_traveller($data, $where)
	{
		return $this->db->update("m_visa_pax", $data, $where);
	}
	
	function delete_traveller($where)
	{
		return $this->db->delete("m_visa_pax", $where);
	}
	...
	{
		$sql = "SELECT {$this->_table}.*, {$this->_table}.flight_number as vb_flight_number, m_visa_pax.*, m_visa_pax.id as pax_id, m_country.id as nation_id, m_country.type as nation_type, m_arrival_port.id as airport_id FROM {$this->_table} INNER JOIN m_visa_pax ON (m_visa_pax.book_id = {$this->_table}.id) INNER JOIN m_country ON (m_visa_pax.nationality = m_country.name) INNER JOIN m_arrival_port ON (m_arrival_port.short_name = {$this->_table}.arrival_port) WHERE 1=1";
		if (!is_null($info)) {
			if (!empty($info->id)) {
			...
			if (!empty($info->agents_id)) {
				$sql .= " AND m_visa_pax.agents_id = '{$info->agents_id}'";
			}
			if (!empty($info->agents_fc_id)) {
				$sql .= " AND m_visa_pax.agents_fc_id = '{$info->agents_fc_id}'";
			}
			if (!empty($info->send_approved)) {
				$sql .= " AND m_visa_pax.send_approved = '{$info->send_approved}'";
			}
			if (!empty($info->send_pickup)) {
				$sql .= " AND m_visa_pax.send_pickup = '{$info->send_pickup}'";
			}
			if (!empty($info->visit_purpose)) {
				$sql .= " AND {$this->_table}.visit_purpose = '{$info->visit_purpose}'";
			}
			...
			if (!empty($info->send_urgent)) {
				$sql .= " AND m_visa_pax.send_urgent = '{$info->send_urgent}'";
			}
			if (!empty($info->from_send_approved_date)) {
				$sql .= " AND m_visa_pax.send_approved_date >= '{$info->from_send_approved_date}'";
			}
			if (!empty($info->to_send_approved_date)) {
				$sql .= " AND m_visa_pax.send_approved_date <= '{$info->to_send_approved_date}'";
			}
			if (!empty($info->status)) {
