...
class M_comment extends M_db
{
	public function __construct()
	{
		...
		$this->_table = "m_comment";
	}
	
	function items($info=NULL, $active=NULL, $limit=NULL, $offset=NULL, $orderby=NULL, $sortby=NULL)
	{
		$sql = "SELECT {$this->_table}.*, m_users.user_fullname, m_users.avatar FROM {$this->_table} INNER JOIN m_users ON (m_users.id = {$this->_table}.user_id) WHERE 1=1";
		if (!is_null($info)) {
