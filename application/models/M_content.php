...
class M_content extends M_db
{
	...
	function jurisdiction_items($info=NULL, $active=NULL, $limit=NULL, $offset=NULL)
	{
		$sql  = "SELECT * FROM {$this->_table} WHERE 1 = 1";
		if (!is_null($info)) {
			if (!empty($info->catid)) {
			...
			if (!empty($info->arr_modules)) {
				foreach ($info->arr_modules as $value) {
					$sql .= " AND id <> '{$value}'";
				}
			}
		}
	...
	{
		$sql_1   = "SELECT * FROM {$this->_table} WHERE id = '{$excluded_id}' AND active = '1'";
